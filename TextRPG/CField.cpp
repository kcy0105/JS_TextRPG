...
void CField::Initialize()
{
	...
	for (int i = 0; i < iMapYLen; ++i)
	{
		for (int j = 0; j < iMapXLen; ++j)
		{
			if (i * iMapXLen + j < iMonCnt)
				m_Map[i][j] = new CMonster(rand()%2); // 몬스터 난이도 랜덤 생성
			else
				m_Map[i][j] = nullptr;
		}
	}
	...
	m_Map[iMapYLen / 2][1] = m_pPlayer; // 이후 맵 해제 시 얕은 복사 주의할 것.
...
bool CField::OnMovement(int targetPosX, int targetPosY) // return 0: 넘어가기 목적, return 1: 상위함수 종료 목적
{
	...
	else if (m_Map[targetPosY][targetPosX]->GetType() == ET_MONSTER)
	{
		...
		while (bloop)
		{
			...
			{
			case 1:
				fightResult = Fight(); // 전투
				if (fightResult == 0)
				{
					return 0;
				}
				else if (fightResult == 1) // 몬스터 처치 시
				{
					SAFE_DELETE(m_Map[targetPosY][targetPosX]); // 몬스터 데이터 해제
				}
				else if (fightResult == 2) // 사망 시 마을로 보내기
				{
					// 회복은 전투 이후? 혹은 여기?
					Release();
					return 1;
